...
//		boolean thereIsTermId = false;
//		if (termIdP != null) {
//			thereIsTermId = true;
//		}

		if (termIdP != null) {// (thereIsAction && !actionP.equals("new_term")) {
			termId = Integer.parseInt(termIdP);
			session.setAttribute("term", daRetrieve.getTerm(termId));
...
//		 } 
		 ...
		 if (termId > 0) { 
			 for (int i=0; i < terms.size(); i++) { 
				 if ( termId == ((Term)terms.get(i)).getTermId() ) { 
					 ...
					 session.setAttribute("term_description", ((Term)terms.get(i)).getTermDescription()); break; 
				 } 
			 } 
		 } else { 
			 //		 exception/error like 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 5 edit_tenglish edit_tbulgarian edit_trussian edit_tgerman
		// edit_tfrench
		...
		if (/* modeS.equals("new") && */ actionS.equals("new_term")) {
			...
			String itemValue = "";
			// switch (actionS) {
			if (actionS.endsWith("_tstandard"))
				itemValue = request.getParameter("t_standardId");
			else if (actionS.endsWith("_tdescription"))
				itemValue = request.getParameter("t_description");
			else if (actionS.endsWith("_tenglish"))
				itemValue = request.getParameter("t_english");
			else if (actionS.endsWith("_tbulgarian"))
				itemValue = request.getParameter("t_bulgarian");
			else if (actionS.endsWith("_trussian"))
				itemValue = request.getParameter("t_russian");
			else if (actionS.endsWith("_tgerman"))
				itemValue = request.getParameter("t_german");
			else if (actionS.endsWith("_tfrench"))
				itemValue = request.getParameter("t_french");
			// default: xxx; break;
			// }//switch
			daCud.editTerm(actionS, termS.getTermId(), itemValue);
			session.setAttribute("term", daRetrieve.getTerm(termS.getTermId()));
		} // if, is not new_term
			// sp_update_term("+termId+", '"+action+"', "+languageId+",
			// '"+itemName+"', '"+valueEbrgfDs+"')
		 ...
		 * 
		 * if (actionP != null && !actionP.equals("")) { if
		 * (actionP.equals("edit")) { System.out.println("edit"); } else if
		 * (actionP.equals("new")) { System.out.println("new"); } else if
		 * (actionP.equals("delete")) { System.out.println("delete"); } else if
		 * (actionP.equals("delete_confirmed")) { System.out.println("delete");
		 * } }
		 */
		...
		// session.setAttribute("standard",
		// dbProv.getStandard(standardS.getStandardId()));
		// session.removeAttribute("term");
