...
public class TermListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("languages", dbProv.getLanguages());
    	...
    	//if (symbolsToSearchS.equals("null") || (symbolsToSearchS.length() < 3) ) {
    	//	symbolsToSearchS = "втулка"; //null;
    	//}    	
    	String termLanguageIdP = ""; int termLanguageId = 0;
    	String termIdP = ""; int termId = 0;
        ...
        if (!(termLanguageId > 0 && termId > 0) ) {
    	  ...
    	  adaptTerm(dbProv, termsS, termLanguageId, termId);
    	}
	    ...
	    if (unlmP != null && unlmP.equals("true")) {	
	      ...
	      if (countPerPageStrP != null) { session.setAttribute("countPerPage", countPerPageStrP); }
	      if (pageNumStrP != null)      { session.setAttribute("pageNum", pageNumStrP); }
	      else { session.setAttribute("pageNum", 1); }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//if (makeNewSearch) {
	    	...
	    	//session.setAttribute("terms", dbProv.getTerms(languageIdS, columnToSearchS, symbolsToSearchS, 1, 1));
	    	...
	    	logAction("setTermsTable action", request.getRemoteUser(), request.getRemoteHost(), 
	    			...
	    			String.valueOf(resultsCountS), request.getRemoteAddr(), "info4",
	...
	void adaptTerm(DbProvider dbProv, TermList terms, int termLanguageId, int termId) {
		//TermList terms = (TermList) session.getAttribute("terms");
	    if (termId > 0) {
	      Term oldTerm = null; 
...
//    	int resultsCount = dbProv.getResultsCount(languageId, columnToSearch, symbolsToSearch);
//    	double pagesCount = Math.ceil(resultsCount / resultsCountOnPage);
		...
		if (String.valueOf(session.getAttribute("unlm")).equals("true") ) {
			...
			if (countPerPageObjS != null) { countPerPageNum = Integer.parseInt(String.valueOf(countPerPageObjS)); }
			int selectedPageNum=1;
			String selectedPageStrS = String.valueOf(session.getAttribute("pageNum"));
			if (!selectedPageStrS.equals("null")) { selectedPageNum = Integer.parseInt(selectedPageStrS); } 
			session.setAttribute("terms", dbProv.getTermsAll(resultsCount, countPerPageNum, selectedPageNum, languageId, symbolsToSearch));
		} else {
			session.setAttribute("resultsCount", dbProv.getResultsCount(languageId, columnToSearch, symbolsToSearch, false) );
			session.setAttribute("terms", dbProv.getTerms(languageId, columnToSearch, symbolsToSearch, 1, 1, false));
		}
	}
	
	private void logAction( String action, String name, String ip, String location,
		String symbolsToSearch, String language, String resultsCntStr, String info3, String info4,
		Date dateTime, BigInteger dateTimeInt, String dateTimeStr ) {
//		"setTermsTable action", "noName", "no ip", "location" + request.getLocale().getCountry(), 
//		symbolsToSearchS, String.valueOf(languageIdS), 
//		String.valueOf(resultsCount), String.valueOf(System.currentTimeMillis())
		DbProvider dbProv = new DbProvider();
		dbProv.logActionWebuserInfo(action, name, ip, location, symbolsToSearch, language, resultsCntStr, info3, info4, 
				dateTime, dateTimeInt, dateTimeStr);
	}
}
